...
[🇺🇸 English](README.md) · [🇳🇱 Nederlands](README.nl.md) · [🇫🇷 Français](README.fr.md) · [🇩🇪 Deutsch](README.de.md) · [🇮🇩 Bahasa Indonesia](README.id.md) · [🇮🇹 Italiano](README.it.md) · [🇲🇾 Bahasa Melayu](README.ms.md) · [🇵🇱 Polski](README.pl.md) · [🇧🇷 Português](README.pt.md) · [🇪🇸 Español](README.es.md) · [🇸🇪 Svenska](README.sv.md) · [🇻🇳 Tiếng Việt](README.vi.md) · [🇨🇳 简体中文](README.zh-cn.md) · **🇹🇼 繁體中文** · [🇯🇵 日本語](README.ja.md) · [🇰🇷 한국어](README.ko.md) · [🇮🇳 हिन्दी](README.hi.md) · [🇷🇺 Русский](README.ru.md) · [🇺🇦 Українська](README.uk.md) · [🇹🇭 ไทย](README.th.md) · [🇬🇷 Ελληνικά](README.el.md) · [🇸🇦 العربية](README.ar.md) · [🇮🇱 עברית](README.he.md)

🔗 **線上展示：** https://mikeminhxi.github.io/CCA-F-Study-Suite/

一個用於備考 Anthropic **Claude Certified Architect — Foundations（CCA-F）** 認證考試的自包含靜態應用。無需構建、無需服務器、無需安裝任何依賴——下載整個資料夾後,在瀏覽器中打開 `index.html` 即可使用。

## 打開方式

雙擊 [index.html](index.html),或用任意靜態文件服務器托管它。學習進度會通過 `localStorage` 保存在本地瀏覽器中,按設備/瀏覽器區分。請將 `style.css` 和 `content.js` 與 `index.html` 放在一起——應用程式需要這兩者齊全;若單獨打開 `index.html`,將只會顯示錯誤,而不是正常載入。若要使用英文以外的語言,還需加上 `translations/` 資料夾——缺少該資料夾時,應用程式會自動回退為英文,不會顯示錯誤。
...
- **EN / NL / FR / DE / ID / IT / MS / PL / PT / ES / SV / VN / ZH / TW / JA / KO / HI / RU / UK / TH / EL / AR / HE 語言切換**——一層運行時文本替換機制,可將界面文案在英語、荷蘭語、法語、德語、印尼語、義大利語、馬來語、波蘭語、葡萄牙語、西班牙語、瑞典語、越南語、簡體中文、繁體中文、日語、韓語、印地語、俄語、烏克蘭語、泰語、希臘語、阿拉伯語與希伯來語之間無需刷新即可切換;所選語言會在下次訪問時保留。
...
- **零外部依賴**——純原生 JS/CSS/SVG,不依賴任何框架或構建工具;唯一的外部請求是 Google Fonts(Space Grotesk、Inter、JetBrains Mono)。
