...
- **Korean (한국어)** — full 531-key dictionary translated and staged at `translations/ko.json` via `fetch-language-dictionary`, on branch `feat/add-korean-language`. Not yet wired into `cca-f-study-suite.html` — that's the `add-language` step, still pending.
