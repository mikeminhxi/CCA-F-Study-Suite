...
- **Alphabetical-by-English-name slot inside the Latin group**: "Swedish" sorts immediately after "Spanish" and before "Vietnamese", so it lands between Español and Tiếng Việt in the dropdown, every README's switch-link row, and the Features-bullet abbreviation list — reusing the existing `sortHint: 'latin'` value, same pattern as every other Latin-script round since Dutch opened the front of the group.
- Dropdown option and translation dictionary were pre-staged before this pass; this round's work was the remaining mechanical wiring — the switch-link row and Features-bullet language lists across all 19 existing READMEs, plus authoring the new `README.sv.md`.
...
- **`index.html` split further: CSS moved to `style.css`, static content to `content.js`.** The `<style>` block (~65KB) and the nine read-only data constants (`QDATA`'s 156-question bank plus `DOMAIN_INFO`, `TASK_STATEMENTS`, `CONCEPTS`, `DOMAIN_DESC`, `RULES`, `CORE`, `KEYWORDS`, `DONATIONS` — ~241KB) now live in their own first-party files, loaded via `<link rel="stylesheet">` and `<script src>`. `index.html` drops from ~431KB to ~116KB, leaving it as app engine and markup only. Content was moved byte-identically — nothing was rewritten. Each file now caches independently, so editing an exam question no longer invalidates the CSS and vice versa.
...
- **A visible error banner when content.js fails to load.** Previously any startup exception was only logged to the console and the app rendered empty panes. `index.html` needs `style.css` and `content.js` beside it — `translations/` is optional and silently falls back to English if missing. If `content.js` specifically is missing, the banner names it directly; any other startup error shows a generic "failed to start" message instead.
...
- **New script-family group established**: Thai is the app's first Thai-script language — not Latin, CJK, Devanagari, or Cyrillic. Reused the Hindi round's (PR #16) trailing-script-group ordering precedent: Thai is appended as a new group at the very end of the dropdown/READMEs, after Cyrillic (Русский), rather than merged into an existing group. Introduced a new `sortHint: 'thai'` value for the staged-dictionary schema.
- **Word-spacing decision, verified rather than assumed**: `noSpaceBeforeUnit: false` for Thai — Thai omits spaces between words within a sentence generally (unlike Latin languages), but standard Thai typographic convention *does* insert a space between a numeral and a following classifier/unit word (e.g. "12 ข้อ", not "12ข้อ"), which is why this isn't CJK-style tight packing despite Thai also lacking word-spacing in general prose. Verified via the rendered `"0 ข้อพร้อมใช้งาน"` badge in browser testing.
- **Font-coverage check**: the app's loaded Google Fonts (Space Grotesk, Inter, JetBrains Mono) don't include Thai glyphs, same as they don't cover CJK/Devanagari/Cyrillic. Confirmed via browser verification that system-font fallback renders Thai correctly with no missing-glyph ("tofu") boxes — no font-loading changes were needed, consistent with every other non-Latin language shipped so far.
- **Standing quality gate**: pre-injection English-word-overlap diff against `de.json` flagged 4 keys; genuine fixes included a bare "sibling" and several instances of "implement" left untranslated (caught by a broader manual sweep beyond just the automated 3-word-threshold gate, since only 2 of the 6 "implement" instances tripped the threshold) — a reminder that the automated gate alone isn't sufficient, per the standing lesson from the Italian round.
- Verified via headless Edge driven over the DevTools Protocol: Study Console (brand/nav/tabs), Concepts tab, Study Hub decision-rule row, Exam by Domain screen (`"0 ข้อพร้อมใช้งาน"` dynamic-format badge, "เริ่มสอบ" start button), and the donate button (`"☕ เลี้ยงกาแฟผมสักแก้ว"`) all render correctly in Thai with zero console errors.
- **Unrelated tooling note found during this round**: headless Edge launched with a fresh `--user-data-dir` still picked up the OS-level signed-in Microsoft account and displayed a "syncing browsing data across devices" prompt on some verification runs, before `--disable-sync --disable-features=msEdgeSyncPromo` was added to the launch flags. Only local `file://` visits were involved. Flagged to the maintainer directly when found; this is not an app change but is recorded here as a note for future verification rounds using this same headless-Edge approach.
...
- **Translated independently from English, not derived from Indonesian**: Malay (Bahasa Melayu) and Indonesian (Bahasa Indonesia) are distinct standardized Malayic languages despite mutual intelligibility. Verified the staged dictionary and all READMEs use genuinely distinct Malay vocabulary/spelling rather than lightly-edited Indonesian (e.g. "muat turun" vs Indonesian "unduh" for download, "betul" vs "benar" for correct, "soalan" vs "soal" for questions).
- **Self-report verification caught nothing wrong, but was worth checking directly**: the translating agent's report claimed the literal exam-stem phrases in the Cheat & Keywords decision-rules table ("IF THE STEM SAYS" column) were translated in German/Italian and therefore should be in Malay too — this reasoning was wrong (those specific stems are deliberately left in English across every language, confirmed by direct lookup against `de.json`/`it.json`), but the actual generated `translations/ms.json` file correctly had no keys for those stems regardless. Lesson reaffirmed: verify an agent's self-reported reasoning against the actual output and sibling files directly, don't trust the stated justification even when the output itself turns out fine.
- Verified via headless Edge driven over the DevTools Protocol: Learning Path, Cheat & Keywords, Concepts tab, a full live exam round (including the "Exam Domains" heading and "N questions available" counter from the prior round's fix), and the donate modal all render correctly in Malay with zero console errors.
...
- **Pre-injection quality gate**: ran an English-word-overlap diff against the German sibling before injecting into `index.html`; found and fixed 7 genuine under-translations (an entirely-untranslated `"(retryable, cause)"`, "pattern", "other", "Reliability" ×4, "confidence" ×5, "level", an inconsistent "(over-engineering)", and a literal `.claude/rules/` header). One judgment call is flagged rather than silently decided: "error" was kept as an English loanword throughout (e.g. "Error Handling" → "Penanganan Error") on the reasoning that Indonesian technical writing commonly retains it unlike "context" (which is always translated to "konteks") — a reviewer may want to revisit this.
- **Pre-existing gap found, not introduced by this round**: the exam-setup screen's "Exam Domains" heading and the dynamic "N questions available" counter are hardcoded English strings set via direct `textContent` assignment, bypassing the i18n dictionary entirely — confirmed missing from all 13 languages' dictionaries, not just Indonesian. Left as-is (out of scope for a language addition); a fix belongs in `update-translations`.
- Verified via headless Edge, driven directly over the DevTools Protocol (Node/Playwright/chromium-cli unavailable in this environment): Learning Path, Cheat & Keywords core-principle cards, Concepts tab, a full live exam round (question counter, sidebar unit label), and the donate button/modal all render correctly in Indonesian with zero console errors across all 5 Study Console tabs.
